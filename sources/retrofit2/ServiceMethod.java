    ...
    static final class Builder<T, R> {
        ...
        public ServiceMethod build() {
            ...
            if (typeResponseType == Response.class || typeResponseType == okhttp3.Response.class) {
                throw methodError(PunctuationConst.SINGLE_QUOTES + Utils.getRawType(this.responseType).getName() + "' is not a valid response body type. Did you mean ResponseBody?", new Object[0]);
            }
        ...
        private ParameterHandler<?> parseParameterAnnotation(int i, Type type, Annotation[] annotationArr, Annotation annotation) {
            ...
            if (annotation instanceof Part) {
                ...
                Headers headersM1926of = Headers.m1926of("Content-Disposition", "form-data; name=\"" + strValue5 + PunctuationConst.DOUBLE_QUOTES, "Content-Transfer-Encoding", part.encoding());
                if (Iterable.class.isAssignableFrom(rawType8)) {
                    if (!(type instanceof ParameterizedType)) {
                        throw parameterError(i, rawType8.getSimpleName() + " must include generic type (e.g., " + rawType8.getSimpleName() + "<String>)", new Object[0]);
                    }
                    ...
                    return new ParameterHandler.Part(headersM1926of, this.retrofit.requestBodyConverter(parameterUpperBound4, annotationArr, this.methodAnnotations)).iterable();
                }
                if (rawType8.isArray()) {
                    ...
                    return new ParameterHandler.Part(headersM1926of, this.retrofit.requestBodyConverter(clsBoxIfPrimitive, annotationArr, this.methodAnnotations)).array();
                }
                if (MultipartBody.Part.class.isAssignableFrom(rawType8)) {
                    throw parameterError(i, "@Part parameters using the MultipartBody.Part must not include a part name in the annotation.", new Object[0]);
                }
                return new ParameterHandler.Part(headersM1926of, this.retrofit.requestBodyConverter(type, annotationArr, this.methodAnnotations));
            }
