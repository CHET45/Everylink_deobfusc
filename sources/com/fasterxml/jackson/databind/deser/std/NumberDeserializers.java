            ...
            if (iCurrentTokenId == 1) {
                ...
                if (iCurrentTokenId != 6) {
                    if (iCurrentTokenId == 7) {
                        ...
                        int i = C14351.$SwitchMap$com$fasterxml$jackson$databind$cfg$CoercionAction[coercionActionFindCoercionAction.ordinal()];
                        if (i == 1) {
    ...
    public static class DoubleDeserializer extends PrimitiveOrWrapperDeserializer<Double> {
        ...
        protected final Double _parseDouble(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException {
            ...
            if (NumberInput.looksLikeValidNumber(strTrim)) {
                jsonParser.streamReadConstraints().validateFPLength(strTrim.length());
                try {
                    return Double.valueOf(_parseDouble(strTrim, jsonParser.isEnabled(StreamReadFeature.USE_FAST_DOUBLE_PARSER)));
                } catch (IllegalArgumentException unused) {
                }
            }
