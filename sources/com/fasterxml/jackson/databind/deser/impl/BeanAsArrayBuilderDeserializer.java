                        ...
                        if (propertyValueBufferStartBuilding.assignParameter(settableBeanPropertyFindCreatorProperty, settableBeanPropertyFindCreatorProperty.deserialize(jsonParser, deserializationContext))) {
                            try {
                                ...
                                if (objDeserializeSetAndReturn.getClass() != this._beanType.getRawClass()) {
                                    return deserializationContext.reportBadDefinition(this._beanType, String.format("Cannot support implicit polymorphic deserialization for POJOs-as-Arrays style: nominal type %s, actual type %s", ClassUtil.getTypeDescription(this._beanType), objDeserializeSetAndReturn.getClass().getName()));
                                }
                            } catch (Exception e2) {
