...
class PathInterpolatorDonut implements Interpolator {
    ...
    private final float[] f547mX;

    /* JADX INFO: renamed from: mY */
    private final float[] f548mY;
    ...
    public PathInterpolatorDonut(Path path) {
        ...
        int i2 = i + 1;
        this.f547mX = new float[i2];
        this.f548mY = new float[i2];
        float[] fArr = new float[2];
        for (int i3 = 0; i3 < i2; i3++) {
            pathMeasure.getPosTan((i3 * length) / i, fArr, null);
            this.f547mX[i3] = fArr[0];
            this.f548mY[i3] = fArr[1];
        }
    }
    ...
    public float getInterpolation(float f) {
        ...
        int length = this.f547mX.length - 1;
        int i = 0;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f547mX[i2]) {
        ...
        float[] fArr = this.f547mX;
        float f2 = fArr[length];
        float f3 = fArr[i];
        float f4 = f2 - f3;
        if (f4 == 0.0f) {
            return this.f548mY[i];
        }
        float f5 = (f - f3) / f4;
        float[] fArr2 = this.f548mY;
        float f6 = fArr2[i];
