...
public final class SpecialCharUtil {
    ...
    public static String filterSpecial(String str) {
        ...
        char[] charArray = str.replaceAll(EMOJI_PATTERN_STR, "").toCharArray();
        StringBuilder sb = new StringBuilder();
        for (char c : charArray) {
            String strValueOf = String.valueOf(c);
            if ("-".equals(strValueOf) || PunctuationConst.UNDERLINE.equals(strValueOf)) {
                sb.append(c);
            } else if (!isSpecialChar(strValueOf) && !isPunctuation(strValueOf)) {
                sb.append(c);
            }
        }
        return sb.toString();
