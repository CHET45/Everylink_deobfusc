...
public class COSXmlSignSourceProvider implements QCloudSignSourceProvider {
    ...
    private final List<String> needToSignHeaders = Arrays.asList("cache-control", "content-disposition", "content-encoding", "content-length", "content-md5", "content-type", "expect", "expires", "host", "if-match", "if-modified-since", "if-none-match", "if-unmodified-since", ClientData.KEY_ORIGIN, "range", "transfer-encoding", "pic-operations");
    ...
    public <T> String source(HttpRequest<T> httpRequest) throws QCloudClientException {
        String strHeadersStringForKeys;
        ...
        for (String str : httpRequest.headers().keySet()) {
            ...
            if (this.needToSignHeaders.contains(lowerCase) || lowerCase.startsWith(Headers.COS_PREFIX) || lowerCase.startsWith("x-ci-")) {
                hashSet.add(str);
            }
        }
        ...
        if (this.parametersRequiredToSign.size() < 1) {
            ...
            Iterator<String> it2 = httpRequest.getNoSignParams().iterator();
            while (it2.hasNext()) {
        ...
        if (this.headerKeysRequiredToSign.size() > 0) {
            ...
            if (lowerCase2 != null && lowerCase2.contains("Content-Type".toLowerCase(Locale.ROOT)) && httpRequest.getRequestBody() != null && !httpRequest.headers().containsKey("Content-Type") && (strContentType = httpRequest.contentType()) != null) {
        ...
        if (mapHeaders == null) {
            strHeadersStringForKeys = "";
        } else {
            strHeadersStringForKeys = headersStringForKeys(mapHeaders, this.headerKeysRequiredToSign, this.headerKeysSigned);
        }
        sb.append(strHeadersStringForKeys);
        sb.append("\n");
        ...
        for (String str2 : linkedList) {
            ...
            if (list != null) {
                for (String str3 : list) {
                    ...
                    set2.add(str2.toLowerCase(Locale.ROOT));
