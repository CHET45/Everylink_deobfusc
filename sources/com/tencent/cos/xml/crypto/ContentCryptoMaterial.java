...
final class ContentCryptoMaterial {
    ...
    static ContentCryptoMaterial fromObjectMetadata(ObjectMetadata objectMetadata, EncryptionMaterialsAccessor encryptionMaterialsAccessor, Provider provider, boolean z, QCLOUDKMS qcloudkms) throws JSONException, CosXmlClientException {
        return fromObjectMetadata0(objectMetadata, encryptionMaterialsAccessor, provider, null, z, qcloudkms);
    }

    static ContentCryptoMaterial fromObjectMetadata(ObjectMetadata objectMetadata, EncryptionMaterialsAccessor encryptionMaterialsAccessor, Provider provider, long[] jArr, boolean z, QCLOUDKMS qcloudkms) throws JSONException, CosXmlClientException {
        return fromObjectMetadata0(objectMetadata, encryptionMaterialsAccessor, provider, jArr, z, qcloudkms);
    }

    private static ContentCryptoMaterial fromObjectMetadata0(ObjectMetadata objectMetadata, EncryptionMaterialsAccessor encryptionMaterialsAccessor, Provider provider, long[] jArr, boolean z, QCLOUDKMS qcloudkms) throws JSONException, CosXmlClientException {
        ...
        if (z2) {
            ...
            if (tagLengthInBits > 0 && tagLengthInBits != (i = Integer.parseInt(userMetadata.get(Headers.CRYPTO_TAG_LENGTH)))) {
                throw CosXmlClientException.internalException("Unsupported tag length: " + i + ", expected: " + tagLengthInBits);
            }
        }
        if (z && str3 == null) {
