...
public class ItemDragAndSwipeCallback extends ItemTouchHelper.Callback {
    ...
    public void onSelectedChanged(RecyclerView.ViewHolder viewHolder, int i) {
        if (i == 2 && !isViewCreateByAdapter(viewHolder)) {
            ...
            viewHolder.itemView.setTag(C1376R.id.BaseQuickAdapter_dragging_support, true);
        } else if (i == 1 && !isViewCreateByAdapter(viewHolder)) {
            this.mAdapter.onItemSwipeStart(viewHolder);
            viewHolder.itemView.setTag(C1376R.id.BaseQuickAdapter_swiping_support, true);
        }
    ...
    public void clearView(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        ...
        if (viewHolder.itemView.getTag(C1376R.id.BaseQuickAdapter_dragging_support) != null && ((Boolean) viewHolder.itemView.getTag(C1376R.id.BaseQuickAdapter_dragging_support)).booleanValue()) {
            this.mAdapter.onItemDragEnd(viewHolder);
            viewHolder.itemView.setTag(C1376R.id.BaseQuickAdapter_dragging_support, false);
        }
        if (viewHolder.itemView.getTag(C1376R.id.BaseQuickAdapter_swiping_support) == null || !((Boolean) viewHolder.itemView.getTag(C1376R.id.BaseQuickAdapter_swiping_support)).booleanValue()) {
            return;
        }
        this.mAdapter.onItemSwipeClear(viewHolder);
        viewHolder.itemView.setTag(C1376R.id.BaseQuickAdapter_swiping_support, false);
    }
