...
public class BleBluetooth {
    ...
    private BluetoothGattCallback coreGattCallback = new BluetoothGattCallback() { // from class: com.clj.fastble.bluetooth.BleBluetooth.1
        ...
        public void onConnectionStateChange(BluetoothGatt bluetoothGatt, int i, int i2) {
            ...
            BleLog.m505i("BluetoothGattCallback：onConnectionStateChange \nstatus: " + i + "\nnewState: " + i2 + "\ncurrentThread: " + Thread.currentThread().getId());
        ...
        public void onServicesDiscovered(BluetoothGatt bluetoothGatt, int i) {
            ...
            BleLog.m505i("BluetoothGattCallback：onServicesDiscovered \nstatus: " + i + "\ncurrentThread: " + Thread.currentThread().getId());
            BleBluetooth.this.bluetoothGatt = bluetoothGatt;
            if (i == 0) {
                Message messageObtainMessage = BleBluetooth.this.mainHandler.obtainMessage();
    ...
    public synchronized void clearCharacterCallback() {
        HashMap<String, BleNotifyCallback> map = this.bleNotifyCallbackHashMap;
        if (map != null) {
            map.clear();
        }
        HashMap<String, BleIndicateCallback> map2 = this.bleIndicateCallbackHashMap;
        if (map2 != null) {
            map2.clear();
        }
        HashMap<String, BleWriteCallback> map3 = this.bleWriteCallbackHashMap;
        if (map3 != null) {
            map3.clear();
        }
        HashMap<String, BleReadCallback> map4 = this.bleReadCallbackHashMap;
        if (map4 != null) {
            map4.clear();
        }
    }
    ...
    public synchronized BluetoothGatt connect(BleDevice bleDevice, boolean z, BleGattCallback bleGattCallback, int i) {
        BleLog.m505i("connect device: " + bleDevice.getName() + "\nmac: " + bleDevice.getMac() + "\nautoConnect: " + z + "\ncurrentThread: " + Thread.currentThread().getId() + "\nconnectCount:" + (i + 1));
        if (i == 0) {
    ...
    public synchronized void refreshDeviceCache() {
        ...
        try {
            ...
            BleLog.m505i("exception occur while refreshing device: " + e.getMessage());
            e.printStackTrace();
        }
        if (method != null && (bluetoothGatt = this.bluetoothGatt) != null) {
            BleLog.m505i("refreshDeviceCache, is success:  " + ((Boolean) method.invoke(bluetoothGatt, new Object[0])).booleanValue());
        }
    }
            ...
            switch (message.what) {
                case 1:
                    ...
                    if (BleBluetooth.this.connectRetryCount >= BleManager.getInstance().getReConnectCount()) {
                        ...
                        BleLog.m504e("Connect fail, try reconnect " + BleManager.getInstance().getReConnectInterval() + " millisecond later");
