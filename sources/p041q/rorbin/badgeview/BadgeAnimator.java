...
public class BadgeAnimator extends ValueAnimator {
    ...
    public void draw(Canvas canvas) {
        for (int i = 0; i < this.mFragments.length; i++) {
            int i2 = 0;
            while (true) {
                BitmapFragment[] bitmapFragmentArr = this.mFragments[i];
                if (i2 < bitmapFragmentArr.length) {
                    bitmapFragmentArr[i2].updata(Float.parseFloat(getAnimatedValue().toString()), canvas);
                    i2++;
                }
            }
        }
    ...
    private BitmapFragment[][] getFragments(Bitmap bitmap, PointF pointF) {
        ...
        for (int i = 0; i < bitmapFragmentArr.length; i++) {
            for (int i2 = 0; i2 < bitmapFragmentArr[i].length; i2++) {
                ...
                bitmapFragment.f2073x = f + width2;
                bitmapFragment.f2074y = f2 + height2;
    ...
    private class BitmapFragment {
        ...
        public void updata(float f, Canvas canvas) {
            ...
            this.f2073x += this.random.nextInt(this.maxSize) * 0.1f * (this.random.nextFloat() - 0.5f);
            float fNextInt = this.f2074y + (this.random.nextInt(this.maxSize) * 0.1f * (this.random.nextFloat() - 0.5f));
            this.f2074y = fNextInt;
            float f2 = this.f2073x;
            float f3 = this.size;
            canvas.drawCircle(f2, fNextInt, f3 - (f * f3), this.paint);
        }
    }
}
